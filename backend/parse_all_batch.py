...
def process_one_lot(lot_id_int):
...
def main():
    db = next(get_db())
    lot_ids = [r[0] for r in db.query(Lot.id).all()]
    print(f"Total lots to parse: {len(lot_ids)}")
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {executor.submit(process_one_lot, lid): lid for lid in lot_ids}
        for future in as_completed(futures):
            completed += 1
            if completed % 500 == 0 or completed == total:
                elapsed = time.time() - start_time
                speed = completed / elapsed if elapsed > 0 else 0
                print(f"[batch_parse] Progress: {completed}/{total} ({completed/total*100:.1f}%) Speed: {speed:.1f} lots/sec")
