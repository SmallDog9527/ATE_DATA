...
file_path = "/home/ATE_DATA/ATE_SERVER/backend/app/api/routes/programs.py"
with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

# Replace _build_data_program_list definition
start_marker = "def _build_data_program_list("
...
if start_idx != -1 and end_idx != -1:
    ...
    new_func_code = """def _build_data_program_list(
    db: Session,
    product_name: str,
    days: int,
    ...
    for idx, row in enumerate(rows, 1):
        row["index"] = idx
    return rows"
    content = content[:start_idx] + new_func_code + content[end_idx:]
    print("Replaced _build_data_program_list successfully.")

snap_start = "def update_all_program_changes_snapshot("
snap_idx = content.find(snap_start)
if snap_idx != -1:
    new_snap_code = """def update_all_program_changes_snapshot(db: Session, force_product_name: Optional[ str] = None) -> None:
    ...
    content = content[:snap_idx] + new_snap_code + "\n"
